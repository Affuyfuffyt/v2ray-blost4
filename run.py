...
def start_command(message):
    ...
    bot.clear_step_handler_by_chat_id(chat_id) # 🔥 التنظيف الإجباري للذاكرة عند كل ضغطة Start
    try:
...
def global_back_handler(call):
    ...
    bot.clear_step_handler_by_chat_id(chat_id) # تنظيف أي أوامر إدخال معلقة
    if check_is_admin(chat_id):
